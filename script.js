...
/* ============================================================
   BRANDS SPARKLES — @tsparticles traduit en vanilla canvas
   density ≈ 800 → ~80 particules sur la section
   opacité: 0.05–0.7, mouvement lent aléatoire
============================================================ */
(function initBrandSparkles() {
  const canvas = document.getElementById('brandsCanvas');
  if (!canvas || prefersReducedMotion) return;
  ...
  const particles = Array.from({ length: COUNT }, () => ({
    ...
    y:       rand(0, 1),
    size:    rand(0.4, 2),
    opacity: rand(0.05, 0.5),
    oDir:    Math.random() > 0.5 ? 1 : -1,
    oSpeed:  rand(0.003, 0.012),   // opacitySpeed ÷ 120fps
    vx:      rand(-0.05, 0.05),    // drift lent
    vy:      rand(-0.03, 0.03),
  }));
  ...
  function draw() {
    ...
    particles.forEach(p => {
      ...
      /* Opacity pulse */
      p.opacity += p.oDir * p.oSpeed;
      if (p.opacity > 0.65 || p.opacity < 0.03) {
        p.oDir *= -1;
        p.opacity = Math.max(0.03, Math.min(0.65, p.opacity));
      }
